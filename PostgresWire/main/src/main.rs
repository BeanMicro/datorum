...
impl SimpleQueryHandler for SqliteBackend {
        ...
        C: ClientInfo + Unpin + Send + Sync,
    {
        ...
        if query.to_uppercase().starts_with("SELECT") {
            ...
            while let Ok(Some(row)) = rows_result.next() {
                ...
                for idx in 0..ncols {
                    ...
                    match data {
                        ...
                        ValueRef::Integer(i) => { encoder.encode_field(&i).unwrap(); }
                        ValueRef::Real(f) => { encoder.encode_field(&f).unwrap(); }
                        ValueRef::Text(t) => {
...
impl ExtendedQueryHandler for SqliteBackend {
        ...
        C: ClientInfo + Unpin + Send + Sync,
    {
        ...
        println!("Received extended query: {}", query);
        ...
        if query.to_uppercase().starts_with("SELECT") {
            ...
            // Process all rows and collect them into memory
            let mut rows_result = stmt.query::<&[&dyn rusqlite::ToSql]>(params_ref.as_ref())
                .map_err(|e| PgWireError::ApiError(Box::new(e)))?;
